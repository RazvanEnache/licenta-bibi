...
const transporter = nodemailer.createTransport({
  service: "hotmail",
  auth: {
    user: "insurechain-22@outlook.com",
    pass: "masline1234",
  },
});
...
async function getRecordByAddress(Model, request, response) {
  try {
...
async function getRecordByCNP(Model, request, response) {
  try {
    let record = await Model.findOne({
      where: {
        cnp: request.params.cnp,
      },
    });
    if (record) {
      response.status(200).json(record);
...
async function getUserByPrivateCode(Model, request, response) {
  try {
    let record = await Model.findOne({
      where: {
        privateCode: request.params.privateCode,
      },
    });
    if (record) {
      response.status(200).json(record);
...
async function patchRecordByAddress(Model, request, response) {
  try {
    let record = await Model.findOne({
      where: {
        ethAddress: request.params.address,
...
async function deleteRecordByEthAddress(Model, request, response) {
  try {
    let record = await Model.findOne({
      where: {
        ethAddress: request.params.address,
